...
async function enviarEntrada() {
  const payload = { plate: detectedPlate };
  if (!detectedPlate) return alert("Escanea una matrícula");
  if (navigator.onLine) {
    await fetch('/api/entry', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify(payload) });
    alert("✔️ Entrada registrada online");
  } else {
    saveOffline('entry', payload);
    alert("⚠️ Guardado offline hasta conexión");
  }
}
...
async function enviarSalida() {
  ...
  if (!detectedPlate) return alert("Escanea una matrícula");
  if (navigator.onLine) {
    const res = await fetch('/api/exit', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify(payload) });
...
function syncOfflineData(){
  ...
  store.getAll().onsuccess = async e=>{
    ...
    for(const {type,payload} of actions){
      try{ await fetch(`/api/${type}`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)}); }
      catch{ return; }
    }
